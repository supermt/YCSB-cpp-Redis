...
void
ParseCommandLine(int argc, const char *argv[], ycsbc::utils::Properties &props);

void StatusThread(ycsbc::Measurements *measurements, CountDownLatch *latch,
                  int interval, const std::string &report_file_prefix) {
    ...
    while (1) {
        ...
        if (done) {
            measurements->print_hdr_hist(report_file_prefix);
            break;
        }
...
int main(const int argc, const char *argv[]) {
    ...
    const bool do_transaction = (props.GetProperty("dotransaction", "false") ==
                                 "true");
    ...
    const int status_interval = std::stoi(
            props.GetProperty("status.interval", "10"));
    ...
    if (do_load) {
        ...
        for (int i = 0; i < num_threads; ++i) {
            ...
            std::atomic_int64_t finished = 0;
            client_threads.emplace_back(
                    std::async(std::launch::async, ycsbc::ClientThread, dbs[i], &wl,
                               &finished, total_ops, true, true, !do_transaction, &latch));
        }
    ...
    if (do_transaction) {
        ...
        for (int i = 0; i < num_threads; ++i) {
            ...
            client_threads.emplace_back(
                    std::async(std::launch::async, ycsbc::ClientThread, dbs[i], &wl,
                               &finished, total_ops, false, !do_load, true, &latch));
        }
        assert((int) client_threads.size() == num_threads);
