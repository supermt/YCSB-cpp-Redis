...
namespace ycsbc {
  ...
  public:
    ...
    void print_hdr_hist(const std::string &prefix);
  ...
  private:
    ...
    struct hdr_histogram *histogram[MAXOPTYPE]{};
    FILE *report_files[MAXOPTYPE];

    const std::string header = "Value, Percentile, TotalCount, 1/(1-Percentile)\n";
